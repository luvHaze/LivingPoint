...
- ### Scope 
  ...
  ```
  코루틴이 실행되는 범위로, 
  Globalscope는 앱 전체의 생명 주기를 따른다. 즉, 앱이 실행중이라면 코루틴이 계속 실행될 수 있다. 만약 Activity와 라이프사이클을 같이 하는 scope를 만든다면 activity가 종료되기 전까지만 실행되고, 액티비티가 종료되면 해당 스코프를 사용한 코루틴도 같이 종료된다.
  ```
...
- ### Suspend function 
  ...
  그 후, 해당 함수의 작업이 완료되면 다시 코루틴 블럭으로 돌아와서 해당 함수의 다음부터 이어서 실행된다. 해당 함수의 작업이 완료되기 전까지는 코루틴 블럭 안에서 다음으로 넘어가지 않는다.
  ```
